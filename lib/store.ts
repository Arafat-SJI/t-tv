...
interface AppState {
  ...
  // View
  activeView: "home" | "fifa" | "countries" | "categories" | "languages" | "favorites";
  setActiveView: (
    view: "home" | "fifa" | "countries" | "categories" | "languages" | "favorites"
  ) => void;
...
function loadFavorites(): string[] {
  ...
  try {
    const stored = localStorage.getItem("TowerTV-favorites");
...
function saveFavorites(favorites: string[]) {
  if (typeof window === "undefined") return;
  localStorage.setItem("TowerTV-favorites", JSON.stringify(favorites));
}
