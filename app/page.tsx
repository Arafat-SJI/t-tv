...
export default function HomePage() {
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center min-h-screen relative">
        <div className="neo-lights pointer-events-none fixed inset-0 z-0" aria-hidden="true" />
        <div className="glass-panel rounded-2xl p-10 text-center relative z-10 max-w-md mx-4">
          <h1 className="font-display text-2xl font-bold mb-2">Failed to load channels</h1>
  ...
  return (
    <div className="flex h-screen overflow-hidden relative">
      {/* Neo ambient lighting */}
      <div className="neo-lights pointer-events-none fixed inset-0 z-0" aria-hidden="true" />
      <div className="neo-grid-overlay pointer-events-none fixed inset-0 z-0" aria-hidden="true" />

      <Sidebar
        countries={countries}
        categories={categories}
        languages={languages}
        fifaChannelCount={fifaChannels.length}
      />
          ...
          {isLoading && allChannels.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-6 px-4">
              <div className="relative">
                <div className="absolute inset-0 rounded-full bg-brand/20 blur-2xl scale-150 animate-pulse-brand" />
                <div className="relative w-20 h-20 rounded-full border-4 border-brand/20 border-t-brand animate-spin" />
                <Tv className="absolute inset-0 m-auto h-8 w-8 text-brand" />
              </div>
            ...
            <div className="max-w-[1800px] mx-auto w-full">
              ...
              <div className="p-3 sm:p-4 lg:p-6 pt-4 space-y-8 sm:space-y-10">
                ...
                {/* FIFA World Cup */}
                {!isFiltered && activeView === "fifa" && (
                  <ChannelGrid
                    channels={fifaChannels}
                    isLoading={false}
                    title="⚽ FIFA World Cup — Live Matches"
                    pageSize={24}
                    showLoadMore={fifaChannels.length > 24}
                    emptyMessage="No FIFA World Cup channels found right now. Try again later or browse Sports in Categories."
                  />
                )}
